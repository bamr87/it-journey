...
session:
  ...
  average_score: 75.0
  verdict_counts: { pass: 2, warn: 2, fail: 1 }
  engine_cost_usd: 3.0902
  evidence: walk-evidence.json
  plan: walk-plan.json
  note: >-
    Machine evidence was pre-computed and sealed by the workflow's deterministic
    execute-engine step (agentic_validate.py --mode execute). This agent consumed
    walk-evidence.json / walk-evidence.md as-is and did NOT re-run the engine or
    edit any quest content.
---
