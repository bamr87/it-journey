...
session:
  ...
  note: >-
    Machine execute-mode evidence exists for only 2 of the 5 planned quests;
    the other 3 did not finish in the engine (the child validator hit its
    40-turn ceiling mid-build). Those 3 are reasoned-only in this report and
    carry NO sandbox verdict. Evidence consumed as-sealed from
    ./walk-evidence.json — not re-run, not edited.
---
