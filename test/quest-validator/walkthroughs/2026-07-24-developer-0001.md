---
title: "Walkthrough — Developer · Level 0001 (Web Fundamentals / Apprentice)"
...
session:
  ...
  notes: >-
    Sealed evidence consumed as-is from the workflow's execute-engine step
    (walk-evidence.json). One quest (forge-your-character) produced NO machine
    verdict — the engine hit its max-turns limit and errored — so it is reasoned
    about statically only. Sandbox is a bare temp dir, not an it-journey checkout,
    so repo-scaffold-dependent commands could not run and are labelled honestly.
---
