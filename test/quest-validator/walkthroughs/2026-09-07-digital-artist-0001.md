...
session:
  ...
  note: >-
    ...
    This window mixes two genuinely linked frontend main quests
    (css-styling-basics -> bootstrap-framework, frontmatter-declared) with
    two standalone quests that carry no quest_dependencies at all
    (advanced-markdown, whose own prose points to css-styling-basics but
    whose frontmatter unlocks seo-optimization/jekyll-plugins instead; and
    building-testing-git-init-script) plus one backend-flavored side quest
    (barodybroject-stack-analysis) that also declares zero dependencies.
---
