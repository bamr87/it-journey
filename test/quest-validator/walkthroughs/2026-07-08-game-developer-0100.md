...
behaving exactly as documented. The headline verdict is **warn** — not because
...
- Complete assembled `scripts/ci/verify.py` → **passed** across 5 scenarios:
  (1) clean file → exit 0, empty `findings.jsonl`; (2) missing `author` key → exit 1,
  `fm-required-key` error; (3) no `---` block → `fm-missing` error; (4) malformed
  front matter → handled gracefully, no crash; (5) dead site-absolute link →
  `link-broken` **warning** (does not fail gate), relative dead links ignored.
  **Determinism confirmed**: re-ran twice on identical state → byte-identical
  `findings.jsonl`.
...
behaved exactly as the quest describes… The main gap is an undisclosed real-world
gotcha — the harness's unscoped Markdown glob will immediately flag a plain
README.md — plus the branch-protection step being UI-only with no scriptable
...
- **low · Quest 1 · Chapter 2 branch protection** — promotion to a required check is
  described only via the Settings UI; no scriptable `gh api …/branches/main/protection`
...
- **medium · Quest 2 · Chapter 4, `.github/workflows/ci-cd-pipeline.yml`** — the CI
  ...
  but the repo the learner actually builds in Chapters 1–2 contains only `README.md`
  and `auth.js` — no `package.json` and no npm scripts. A beginner who wires this
  ...
  verified in a sandbox — reasoned from the quest source.)*
...
- **low · Quest 3 · Step 7 build command** — `bundle exec jekyll serve` conflicts with
  ...
  the `run-it-journey` skill is required for the `jekyll-theme-zer0` gem ≥1.21 /
  Ruby ≥3.2). A contributor following this literally on the host may hit a gem/Ruby
...
The three quests share level 0100 but come from **three unrelated series** —
...
and stands alone as a contributor CSS side quest — it neither requires nor is required
...
specific — a CI gate, generic Git, and a profile-theming CSS exercise are shared
...
- **What I ran vs. reasoned:** I ran **nothing** myself against the quests. Quest 1's
  ...
  disposable sandbox, 23 turns, ~$0.80) — I consumed it as-is. Quests 2 and 3 are
...
- **Coverage / truncation (mandatory honesty):** the plan requested 3 quests; the
  ...
  auth_truncated: true`) — the OAuth token rate-limited before Quests 2 and 3 were
