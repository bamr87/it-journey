...
session:
  ...
  auth_truncated: true
  note: >-
    Planner selected 4 linked quests; the sealed execute engine scored only the
    first (aws-essentials) before the OAuth budget truncated the run
    (auth_truncated: true, evaluated 1 of 4 requested). Quests 2–4 were walked by
    static reading only and are labelled `reasoned`, never `tested`.
---
