...
permalink: /quests/codex/self-operating-website-04-the-sigils-of-trust/
fmContentType: quest
layout: quest
draft: false
comments: true
...
```yaml
# Gate the whole job behind a repo variable AND the auth secret.
jobs:
  curate:
    # `vars.*` are repository variables (not secrets); `secrets.*` are encrypted.
    if: >-
      vars.CONTENT_FACTORY_ENABLED == 'true' &&
...
This lesson mirrors the real auth-and-safety work merged into the `bamr87/lifehacker.dev` reference build. Each PR inscribed one sigil:
...
**Objective:** Arm and disarm a real automation, proving the kill switch and least-privilege binding work end to end.
...
- **Next chapter:** [The Content Forge](/quests/codex/self-operating-website-05-the-content-forge/)
...
**Previous:** [[The War Machine: CI/CD Workflows That Run the Realm]]
**Next:** [[The Content Forge: An Agent That Writes and Ships]]
