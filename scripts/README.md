...
│   ├── compress_images.py              # PNG → WebP pass after preview-image runs
...
- Preview images are generated by the `zer0-image-generator` gem (`bundle exec jekyll preview-images`, configured by `preview_images:` in `_config.yml`), not a script here
