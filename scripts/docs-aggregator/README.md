...
Usage: transform.py [--work-dir path] [--output-dir path] [--nav-output path]
  ...
  --nav-output   Optional path for navigation YAML fragment
```
...
- Wargames pages generated by this pipeline use dynamic sidebar navigation via `sidebar.nav: auto`.
- The committed static file `_data/navigation/docs_aggregated.yml` is intentionally not used.
- `transform.py --nav-output ...` can still be used for ad-hoc debugging, but it is optional.
